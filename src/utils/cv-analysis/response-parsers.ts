 ...
 */
export function parseScoresFromResponse(response: string): Record<string, number> {
  try {
 ...
 */
export function parseScoreExplanationsFromResponse(response: string): Record<string, string> {
  try {
    const parsedData = JSON.parse(response);
    const explanations: Record<string, string> = {};
    
    // Extract explanations from scoreExplanations section
    if (parsedData.scoreExplanations) {
      Object.entries(parsedData.scoreExplanations).forEach(([key, explanation]: [string, any]) => {
 ...
 */
export function parseRecommendationsFromResponse(response: string): CVTip[] {
  try {
    const parsedData = JSON.parse(response);
    
    if (Array.isArray(parsedData.recommendations)) {
      return parsedData.recommendations.map((rec: any, index: number) => ({
        id: rec.id || `parsed-rec-${index}`,
        category: rec.category || "",
        title: rec.title || "",
        description: rec.description || "",
        priority: rec.priority || "medium",
        text: rec.text || rec.title || rec.description || "" // Ensure text is always populated
      }));
    }
